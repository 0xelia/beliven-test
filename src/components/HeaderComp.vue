<template>
    <header>
        <!-- logo nav -->
        <nav class="bg-blue-900 px-2 py-3">
        ...
        <div class="container px-2">
            <!-- new project bar -->
            <nav class="flex pt-4 pb-2 justify-between items-center border-0 border-b-[1px] border-b-gray-300">
                <h2 class="text-2xl text-gray-900">Projects</h2>
                <button 
                @click="setVisible()"
                class="text-lg text-white bg-blue-800 px-6 py-1 rounded-md cursor-pointer">
                    <span class="hidden sm:block">New Project</span>
                    <span class="font-bold text-2xl sm:hidden">+</span>
                </button>
            </nav>
            <div class="py-2 flex flex-col items-center sm:flex-row sm:justify-between gap-2 border-b-[1px] border-b-gray-300">
                <!-- filter bar -->
                <nav class="flex gap-3">
                    <template v-for="(link, i) in navLinks" :key="i">
                        <button 
                            @click="setProjects(link)"
                            :class="[link.active ? 'relative px-1 text-white sm:text-black after:rounded-md after:z-0 after:absolute after:h-full sm:after:h-[2px] after:bg-violet-800 sm:after:-bottom-[9px] after:-left-1 after:-right-1' : '', 
                ...
                <!-- search and sort bar -->
                <nav class="flex gap-1 justify-center sm:justify-end basis-full sm:basis-1/2">
                    <input 
                    v-model="query"
                    @input="filterByName(query)"
                    type="search" name="filter_by_name" id="" placeholder="Filter by name"
                    class="border w-1/2 border-gray-300 rounded-md px-1 text-sm py-1" >
                    
                    <select 
                    @input="orderProjects($event.target.value)" name="order_by" id="" class="text-gray-400 py-1 border border-gray-300 w-1/2 rounded-md px-1 text-sm">
